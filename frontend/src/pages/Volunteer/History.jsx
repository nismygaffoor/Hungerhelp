...
const History = () => {
    ...
    const handleStatusUpdate = async (taskId, newStatus) => {
        ...
        const ok = await confirmDialog(t(confirmKey), { variant: 'danger' });
        ...
        try {
            ...
            toast.error(err.response?.data?.message || t('volunteer.history.updateFailed'));
    ...
    const handleRelease = async (taskId) => {
        ...
        const ok = await confirmDialog(t('volunteer.history.releaseConfirm'), { variant: 'danger' });
        ...
        try {
            ...
            const res = await api.post(`/delivery/${taskId}/release`);
            ...
            toast.error(err.response?.data?.message || t('volunteer.history.releaseFailed'));
                    ...
                    <div className="flex flex-col gap-2 w-full lg:w-auto shrink-0">
                        ...
                        {task.status === 'Assigned' && (
                            ...
                            <button
                                ...
                                onClick={() => handleRelease(task._id)}

                                disabled={updatingId === task._id}

                                className="px-6 py-2.5 rounded-xl font-bold text-sm bg-white text-red-600 border border-red-100 hover:bg-red-50 transition-all disabled:opacity-60"

                            >

                                {updatingId === task._id ? t('volunteer.history.updating') : t('volunteer.history.releaseTask')}
