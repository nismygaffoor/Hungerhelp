            ...
            <main className={`flex-1 ml-0 transition-all duration-300 ${isCollapsed ? 'md:ml-16' : 'md:ml-64'} bg-white min-h-screen`}>
                ...
                <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
                    <header className="mb-6 text-left">
                        <h2 className="text-3xl font-bold text-gray-900 leading-tight">{t('beneficiary.requestFoodTitle')}</h2>
                        <p className="text-gray-500 text-sm font-medium mt-1">{t('beneficiary.requestFoodSubtitle')}</p>
                    </header>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        <div className="lg:col-span-8 space-y-6">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="text-lg font-bold mb-4 text-gray-800 flex items-center gap-2">
                                    <Utensils className="text-[#1E5144]" size={20} />
                                    {t('beneficiary.neededItems')}
                                </h3>
                                <div className="space-y-4">
                                    {items.map((item, index) => (
                                        <div key={index} className="flex flex-col md:flex-row items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 group hover:border-green-200 transition-all">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="text-lg font-bold mb-4 text-gray-800 flex items-center gap-2">
                                    <FileText className="text-[#1E5144]" size={20} />
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="text-lg font-bold mb-4 text-gray-800">{t('beneficiary.logistics')}</h3>
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">{t('beneficiary.deliveryAddress')}</label>
                                        <LocationFields
                                            compact
                                            district={district}
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">{t('beneficiary.urgencyLevel')}</label>
                                            ...
                                            {URGENCY_LEVELS.map((level) => (
                                                <button
                                                    ...
                                                    className={`py-2 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest ${
                                                        urgency === level 
                                                        ? 'bg-[#1E5144] border-[#1E5144] text-white shadow-md' 
                            ...
                            <div className="bg-[#1E5144] rounded-2xl p-5 shadow-sm text-white">
                                <div className="flex items-start gap-3">
                                    <div className="p-2 bg-white/10 rounded-xl">
                                        <AlertCircle size={20} className="text-green-200" />
                                    </div>
                                    <div>
                                        <h4 className="text-xs font-black uppercase tracking-widest text-green-200 mb-1">{t('beneficiary.notice')}</h4>
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full bg-[#1E5144] hover:bg-[#163d33] text-white font-bold py-3 rounded-xl shadow-sm transition-all active:scale-95 disabled:opacity-70 flex items-center justify-center gap-2 uppercase tracking-widest text-xs"
                            >
